...
async def process_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message_to_edit = await update.message.reply_text("正在处理...")
    ...
    if action.startswith('new_tag_'):
        ...
        try:
            ...
            await message_to_edit.edit_text(f"✅ 标签 `{text}` 已成功添加！")
        except Exception as e:
            logger.error(f"添加标签失败: {e}")
            await message_to_edit.edit_text(f"❌ 添加失败！标签 `{text}` 可能已存在。")
        # Let user navigate back manually
        ...
        try:
            ...
            if user:
                ...
                await message_to_edit.edit_text(f"✅ 用户 `{user['first_name'] or user['username']}` 已被设为管理员！")
            else:
                await message_to_edit.edit_text("❌ 找不到该用户。")
        except Exception as e:
            logger.error(f"添加管理员失败: {e}")
            await message_to_edit.edit_text("❌ 添加管理员失败！")
        ...
        await message_to_edit.edit_text("✅ 欢迎消息已更新！")
