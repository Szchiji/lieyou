...
async def my_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_cursor() as cur:
        ...
        tags = await cur.fetch("SELECT t.tag_name, COUNT(v.id) as count FROM votes v JOIN tags t ON v.tag_id = t.id WHERE v.nominee_id = $1 GROUP BY t.tag_name", user_id)
    
    if not user_data:
        await update.message.reply_text("似乎还没有你的记录。")
        return
    ...
    # 核心改造：显示推荐数和拉黑数
    text = (f"我的档案:\n"
            f"👍 **收到推荐: {user_data.get('recommend_count', 0)} 次**\n"
            f"👎 **收到拉黑: {user_data.get('block_count', 0)} 次**\n\n"
            f"收到的所有评价标签:\n{tags_str}")
