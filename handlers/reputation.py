...
async def handle_nomination(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # 注意 callback_data 也变成了符号
        keyboard = [[InlineKeyboardButton("👍 推荐", callback_data=f"vote_up_{nominator_id}_{nominee_username}"),
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    action, vote_type, nominator_id_str, nominee_username = data[0], data[1], data[2], "_".join(data[3:])
    ...
    try:
        if action == "vote":
            ...
            tag_id, nominator_id = int(vote_type), int(nominator_id_str) # 复用变量名
            async with db_cursor() as cur:
