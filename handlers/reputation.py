    ...
    if message.entities:
        for entity in message.entities:
            if entity.type == 'mention':
                ...
                target_user_from_entity = entity.user; break
    if not target_user_from_entity and not target_username_from_text and update.effective_chat.type == 'private':
        text = message.text.strip()
        if text.startswith('@'): target_username_from_text = text[1:]
        elif '查询' in text: target_username_from_text = text.replace('查询', '').strip().lstrip('@')
    if not target_user_from_entity and not target_username_from_text: return
...
async def send_reputation_card(message_or_query, context: ContextTypes.DEFAULT_TYPE, target_user_pkid: int, origin: str = ""):
    ...
    try:
        ...
        if not card_data: raise ValueError("无法构建声誉卡片数据")
...
async def process_vote(update: Update, context: ContextTypes.DEFAULT_TYPE, target_user_pkid: int, tag_id: str, origin: str):
    query = update.callback_query
    voter = await get_or_create_user(user_id=query.from_user.id)
    if not voter:
        await query.answer("❌ 无法识别您的身份。", show_alert=True); return
    tag_id = int(tag_id)
