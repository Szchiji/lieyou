...
async def remove_favorite(update: Update, context: ContextTypes.DEFAULT_TYPE, target_user_pkid: int, origin: str):
    ...
    try:
        ...
        await query.answer("💔 已取消收藏。", show_alert=False)
        # 核心修正：如果从声誉卡片发起，则刷新卡片；否则返回收藏列表
        if origin and origin.startswith("fav_"):
             await my_favorites_list(update, context, int(origin.split('_')[1] or 1))
        else:
             await send_reputation_card(update, context, target_user_pkid, origin)
    except Exception as e:
...
async def my_favorites_list(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 1):
    ...
    try:
        ...
        if not favs:
            ...
            for fav in favs:
                display_name = fav.get('first_name') or f"@{fav.get('username')}" or f"用户 {fav['pkid']}"
                callback_data = f"rep_card_query_{fav['pkid']}_fav_{page}"
                keyboard_list.append([
                    InlineKeyboardButton(f"👤 {display_name}", callback_data=callback_data),
                    # 核心修正：从收藏列表发起的取消收藏，也带上 origin
                    InlineKeyboardButton("❌ 取消收藏", callback_data=f"remove_favorite_{fav['pkid']}_fav_{page}")])
